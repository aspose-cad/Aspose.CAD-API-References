---
title: Matrix
second_title: Aspose.CAD for .NET API 参考
description: 将 Matrix 类的新实例初始化为单位矩阵
type: docs
weight: 10
url: /zh/net/aspose.cad/matrix/matrix/
---
## Matrix() {#constructor}

将 Matrix 类的新实例初始化为单位矩阵。

```csharp
public Matrix()
```

### 也可以看看

* class [Matrix](../../matrix)
* 命名空间 [Aspose.CAD](../../matrix)
* 部件 [Aspose.CAD](../../../)

---

## Matrix(float, float, float, float, float, float) {#constructor_3}

初始化[`Matrix`](../../matrix)类的新实例。

```csharp
public Matrix(float m11, float m12, float m21, float m22, float m31, float m32)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| m11 | Single | m00 M11 比例 X |
| m12 | Single | m10 M12 剪切Y |
| m21 | Single | m01 M21 剪切 X |
| m22 | Single | m11 M22 比例 Y |
| m31 | Single | m02 M31 平移 X |
| m32 | Single | m12 M32 平移 Y |

### 也可以看看

* class [Matrix](../../matrix)
* 命名空间 [Aspose.CAD](../../matrix)
* 部件 [Aspose.CAD](../../../)

---

## Matrix(RectangleF, PointF[]) {#constructor_2}

初始化[`Matrix`](../../matrix)类的新实例到由指定矩形和点数组定义的几何变换.

```csharp
public Matrix(RectangleF rect, PointF[] plgpts)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| rect | RectangleF | A[`RectangleF`](../../rectanglef)表示要变换的矩形的结构。 |
| plgpts | PointF[] | 三个数组[`PointF`](../../pointf)表示平行四边形的点的结构矩形的左、右上和左下角将被变换。平行四边形的右下角由前三个角隐含。 |

### 也可以看看

* struct [RectangleF](../../rectanglef)
* struct [PointF](../../pointf)
* class [Matrix](../../matrix)
* 命名空间 [Aspose.CAD](../../matrix)
* 部件 [Aspose.CAD](../../../)

---

## Matrix(Rectangle, Point[]) {#constructor_1}

初始化[`Matrix`](../../matrix)类的新实例到由指定矩形和点数组定义的几何变换.

```csharp
public Matrix(Rectangle rect, Point[] plgpts)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| rect | Rectangle | A[`Rectangle`](../../rectangle)表示要变换的矩形的结构。 |
| plgpts | Point[] | 三个数组[`Point`](../../point)表示平行四边形的点的结构矩形的左、右上和左下角将被变换。平行四边形的右下角由前三个角隐含。 |

### 也可以看看

* struct [Rectangle](../../rectangle)
* struct [Point](../../point)
* class [Matrix](../../matrix)
* 命名空间 [Aspose.CAD](../../matrix)
* 部件 [Aspose.CAD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.CAD.dll -->
