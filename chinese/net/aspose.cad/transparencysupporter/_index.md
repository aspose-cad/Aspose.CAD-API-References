---
title: TransparencySupporter
second_title: Aspose.CAD for .NET API 参考
description: 支持透明度的对象
type: docs
weight: 30410
url: /zh/net/aspose.cad/transparencysupporter/
---
## TransparencySupporter class

支持透明度的对象。

```csharp
public class TransparencySupporter
```

## 构造函数

| 姓名 | 描述 |
| --- | --- |
| [TransparencySupporter](transparencysupporter)() | 默认构造函数。 |

## 特性

| 姓名 | 描述 |
| --- | --- |
| [Opacity](../../aspose.cad/transparencysupporter/opacity) { get; set; } | 获取或设置对象的不透明度。该值应介于 0 和 1 之间。值 0 表示对象完全可见，值 1 表示对象完全不透明。 |

### 也可以看看

* 命名空间 [Aspose.CAD](../../aspose.cad)
* 部件 [Aspose.CAD](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.CAD.dll -->
