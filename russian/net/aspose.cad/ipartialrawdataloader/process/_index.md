---
title: Process
second_title: Справочник по Aspose.CAD для .NET API
description: Обрабатывает загруженные данные.
type: docs
weight: 10
url: /ru/net/aspose.cad/ipartialrawdataloader/process/
---
## Process(Rectangle, byte[], Point, Point) {#process}

Обрабатывает загруженные данные.

```csharp
public void Process(Rectangle rectangle, byte[] data, Point start, Point end)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| прямоугольник | Rectangle | Прямоугольник данных. |
| данные | Byte[] | Необработанные данные. |
| start | Point | Начальная точка данных. Если не равно (слева, сверху), это означает, что это не полный прямоугольник. |
| end | Point | Конечная точка данных. Если не равно (справа, внизу), это означает, что это не полный прямоугольник. |

### Смотрите также

* struct [Rectangle](../../rectangle)
* struct [Point](../../point)
* interface [IPartialRawDataLoader](../../ipartialrawdataloader)
* пространство имен [Aspose.CAD](../../ipartialrawdataloader)
* сборка [Aspose.CAD](../../../)

---

## Process(Rectangle, byte[], Point, Point, LoadOptions) {#process_1}

Обрабатывает загруженные данные.

```csharp
public void Process(Rectangle rectangle, byte[] data, Point start, Point end, 
    LoadOptions loadOptions)
```

| Параметр | Тип | Описание |
| --- | --- | --- |
| прямоугольник | Rectangle | Прямоугольник данных. |
| данные | Byte[] | Необработанные данные. |
| start | Point | Начальная точка данных. Если не равно (слева, сверху), это означает, что это не полный прямоугольник. |
| end | Point | Конечная точка данных. Если не равно (справа, внизу), это означает, что это не полный прямоугольник. |
| loadOptions | LoadOptions | Параметры загрузки. |

### Смотрите также

* struct [Rectangle](../../rectangle)
* struct [Point](../../point)
* class [LoadOptions](../../loadoptions)
* interface [IPartialRawDataLoader](../../ipartialrawdataloader)
* пространство имен [Aspose.CAD](../../ipartialrawdataloader)
* сборка [Aspose.CAD](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.CAD.dll -->
