---
title: LibraryVisualScenes
second_title: Aspose.CAD for .NET API Reference
description: 
type: docs
weight: 4950
url: /net/aspose.cad.fileformats.collada.fileparser.elements/libraryvisualscenes/
---
## LibraryVisualScenes class

The library of visual scenes. The library_visual_scenes element declares a module of visual_scene elements.

```csharp
public class LibraryVisualScenes : ColladaElement
```

## Constructors

| Name | Description |
| --- | --- |
| [LibraryVisualScenes](libraryvisualscenes)() | The default constructor. |

## Properties

| Name | Description |
| --- | --- |
| [Asset](asset) { get; set; } | Gets or sets the asset. The library_visual_scenes element may contain an asset element. |
| [Extra](extra) { get; set; } | Gets or sets the extra. The extra element may appear any number of times. |
| [Id](id) { get; set; } | Gets or sets the id. The id attribute is a text string containing the unique identifier of this element. This value must be unique within the instance document. Optional attribute. |
| [Name](name) { get; set; } | Gets or sets the name. The name attribute is the text string name of this element. Optional attribute. |
| [VisualScene](visualscene) { get; set; } | Gets or sets the visual scene. There must be at least one visual_scene element. |

### See Also

* class [ColladaElement](../colladaelement)
* namespace [Aspose.CAD.FileFormats.Collada.FileParser.Elements](../../aspose.cad.fileformats.collada.fileparser.elements)
* assembly [Aspose.CAD](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.CAD.dll -->
